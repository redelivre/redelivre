...
class Dropbox_OAuth_Curl extends Dropbox_OAuth {
    ...
    public function fetch($uri, $arguments = array(), $method = 'GET', $httpHeaders = array()) {
		...
		if (!empty($errorno))
			...
			throw new Dropbox_Exception_NotFound('Curl error: ('.$errorno.') '."\n");
		...
		if ($status>=300) {
			...
			switch ($status) {
				...
				case 304 :
					...
					break;
				case 403 :
					throw new Dropbox_Exception_Forbidden('Forbidden. This could mean a bad OAuth request, or a file or folder already existing at the target location.' . $body["error"] . "\n");
