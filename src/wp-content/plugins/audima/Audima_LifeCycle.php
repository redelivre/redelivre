...
include_once('Audima_InstallIndicator.php');

class Audima_LifeCycle extends Audima_InstallIndicator
{
    ...
    {
        $plugin = new Audima_Plugin();

        $plugin->otherUninstall();
        $plugin->unInstallDatabaseTables();
        $plugin->deleteSavedOptions();
        $plugin->markAsUnInstalled();
    }
     ...
     * or more complex:
     *    $url = sprintf($this->getAjaxUrl('myaction&id=%s&var2=%s&var3=%s'), urlencode($id), urlencode($var2),
     *     urlencode($var3));
